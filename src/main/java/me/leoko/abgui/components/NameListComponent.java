...
public class NameListComponent extends CustomComponent {
    public NameListComponent(int x, int y, List<String> names, BiConsumer<Interaction, String> nameCallback) {
        final List<Component> textComponents = IntStream.range(0, names.size())
                .mapToObj(i -> new PlayerNameComponent(names.get(i), x, y + i * 18, (context, player, primaryTrigger) -> nameCallback.accept(context, names.get(i))))
                .collect(Collectors.toList());

        this.component = new GroupComponent("", null, textComponents);
    }
}
